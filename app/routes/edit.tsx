...
export default function EditRoute() {
  ...
  // garante string (às vezes pode vir string[])
  ...
  const canSave = useMemo(() => {
    return routeTitle.trim().length >= 3 && normalizedBlocks.length > 0 && !isSaving && !isLoading;
  }, [routeTitle, normalizedBlocks.length, isSaving, isLoading]);

  // carrega rota existente
  useEffect(() => {
    const load = async () => {
      setMsg(null);
      ...
      if (!routeIdStr) {
        setMsg("Rota não encontrada (routeId ausente).");
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        if (!snap.exists()) {
          setMsg("Essa rota não existe mais.");
          return;
        }
        ...
        const loadedBlocks: Block[] = (data.blocks ?? []).map((b: any) => ({
          id: b.id ?? makeId(),
          type: b.type,
          content: b.content ?? "",
          format: {
        ...
        setMsg("Não foi possível carregar. Tente novamente.");
      } finally {
        setIsLoading(false);
      }
    };
        ...
        >
          ...
          <View style={styles.bottomDock}>
            <View style={styles.actionBar}>
              ...
              <TouchableOpacity
                style={styles.actionBtn}
                onPress={() => setMsg("Opção de mapa (em breve).")}
                disabled={isLoading || isSaving}
              >
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.9}
            >
              {isSaving ? <ActivityIndicator /> : <Text style={styles.saveText}>Salvar alterações</Text>}
            </TouchableOpacity>
